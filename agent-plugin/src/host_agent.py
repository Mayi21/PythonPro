...
log_handler = TimedRotatingFileHandler('../logs/vm_agent_app.log', when='midnight', interval=1, backupCount=14)
...
async def async_execute_cmd(command: Cmd):
    ...
    exec_cmd_thread = threading.Thread(target=Utils.async_exec_cmd, args=command.value)
...
async def sync_execute_cmd(command: Cmd):
    print("execute shell cmd is {}".format(command.value))
    out = Utils.sync_exec_cmd(command.value)
    return Response(RequestInfo.SUCCESS_CODE,
                    msg="execute cmd success",
                    data=out)
...
async def execute_shell_file(file_name: str):
    ...
    res = Utils.sync_exec_cmd('sh ' + shell_file_path)['result']
    return Response(RequestInfo.SUCCESS_CODE,
                    msg="success!!",
                    data=json.loads(res))
